...
void SceneTest::load() {
  ...
  target.setOrigin(sf::Vector2f(target.getRadius(), target.getRadius()));
  ...
  seekBehavior.setTarget(
      Vector2f(rnd->getInstance()->getUniformFloat(100.f, 1400.f),
               rnd->getInstance()->getUniformFloat(100.f, 800.f)));

  for (std::size_t i = 0; i < 50; ++i) {
    Vector2f p(rnd->getInstance()->getUniformFloat(100.f, 1400.f),
               rnd->getInstance()->getUniformFloat(100.f, 800.f));
  ...
  target.setPosition(
      sf::Vector2f(seekBehavior.getTarget().x, seekBehavior.getTarget().y));

  wanderBehavior.setDistance(150.f);
  wanderBehavior.setRadius(50.f);
  wanderBehavior.setRandomRange(150.f);

  boid.setPosition(rnd->getUniformFloat(100.f, 1500.f),
...
void SceneTest::handleEvent(sf::Event& event) {
  ...
  if (event.type == sf::Event::MouseButtonPressed) {
    if (event.mouseButton.button == sf::Mouse::Button::Left) {
      seekBehavior.setTarget(
          Vector2f(getMousePosition().x, getMousePosition().y));
      target.setPosition(
          sf::Vector2f(seekBehavior.getTarget().x, seekBehavior.getTarget().y));
    }
  }
