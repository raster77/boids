...
void SceneFlock::load() {
  ...
  for (std::size_t i = 0; i < N; ++i) {
    Boid b;
    b.setRadius(5.f);
    b.setPosition(rnd->getUniformFloat(100.f, 1500.f),
                  rnd->getUniformFloat(100.f, 800.f));
    b.setMaxForce(10.f);
    b.setMaxSpeed(200.f);
    b.addBehavior(&wanderBehavior);
    b.addBehavior(&flockBehavior);
    b.apply(
        Vector2f(rnd->getUniformFloat(-300.f, 300.f),
                 rnd->getUniformFloat(-300.f, 300.f)));
...
void SceneFlock::update(const float dt) {
  ...
  if (mRunning) {
    ...
    idx = 0;
    for (auto &b : boids) {
      std::vector<Boid*> res = grid.get(sprites[idx].getBoundingBox().left,
                                        sprites[idx].getBoundingBox().top);
      ...
      vertices.insert(vertices.end(), sprites[idx].getVertices().begin(),
                      sprites[idx].getVertices().end());
      if (showTrail)
        trailVertices.insert(trailVertices.end(),
                             sprites[idx].getTrailVertices().begin(),
                             sprites[idx].getTrailVertices().end());
      idx++;
    }
    times[1].stop();
...
void SceneFlock::draw() {
  ...
  if (showTrail)
    window->draw(trailVertices.data(), trailVertices.size(), sf::Triangles);
...
void SceneFlock::checkBounds(Boid& b) {
  const float x =
      b.getPosition().x < 0.f ? window->getSize().x :
      b.getPosition().x > window->getSize().x ? 0.f : b.getPosition().x;
  const float y =
      b.getPosition().y < 0.f ? window->getSize().y :
      b.getPosition().y > window->getSize().y ? 0.f : b.getPosition().y;
  b.setPosition(x, y);
}
